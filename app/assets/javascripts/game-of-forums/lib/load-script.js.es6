...
export default function loadScript(url, opts) {
  ...
  return new Ember.RSVP.Promise(function(resolve) {
    url = GameOfForums.getURL((assetPath && assetPath(url)) || url);
    ...
    if (GameOfForums.CDN && url[0] === "/" && url[1] !== "/") {
      cdnUrl = GameOfForums.CDN.replace(/\/$/,"") + url;
    }
    ...
    if (opts.scriptTag) {
      ...
      GameOfForums.ajax({url: cdnUrl, dataType: "script", cache: true}).then(cb);
    }
  });
}
