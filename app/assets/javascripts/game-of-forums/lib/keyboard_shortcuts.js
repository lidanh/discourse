...
GameOfForums.KeyboardShortcuts = Ember.Object.createWithMixins({
  ...
  createTopic: function() {
    GameOfForums.__container__.lookup('controller:composer').open({action: GameOfForums.Composer.CREATE_TOPIC, draftKey: GameOfForums.Composer.CREATE_TOPIC});
  },

  pinUnpinTopic: function() {
    GameOfForums.__container__.lookup('controller:topic').togglePinnedState();
  },

  toggleProgress: function() {
    GameOfForums.__container__.lookup('controller:topic-progress').send('toggleExpansion', {highlight: true});
  },

  showSearch: function(selectContext) {
    $('#search-button').click();
    if(selectContext) {
      GameOfForums.__container__.lookup('controller:search').set('searchContextEnabled', true);
    }
    return false;
  },
  ...
  showHelpModal: function() {
    GameOfForums.__container__.lookup('controller:application').send('showKeyboardShortcutsHelp');
  },
  ...
  _moveSelection: function(direction) {
    ...
    if ($article.size() > 0) {
      ...
      if (rgx === null || typeof rgx[1] === 'undefined') {
        ...
        GameOfForums.URL.jumpToPost(rgx[1]);
      }
    }
  },
