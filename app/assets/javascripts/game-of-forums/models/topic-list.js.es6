...
function topicsFrom(result, store) {
  ...
  const categories = GameOfForums.Category.list(),
        users = Model.extractByKey(result.users, GameOfForums.User);
...
const TopicList = RestModel.extend({
  ...
  loadMore: function() {
    ...
    if (moreUrl) {
      ...
      const store = this.store;
      return GameOfForums.ajax({url: moreUrl}).then(function (result) {
        let topicsAdded = 0;
        ...
        if (result) {
          ...
          GameOfForums.Session.currentProp('topicList', self);
  ...
  loadBefore: function(topic_ids){
    ...
    const url = GameOfForums.getURL("/") + this.get('filter') + "?topic_ids=" + topic_ids.join(",");

    const store = this.store;
    return GameOfForums.ajax({ url }).then(function(result) {
      let i = 0;
      ...
      GameOfForums.Session.currentProp('topicList', topicList);
    });
  }
});
...
TopicList.reopenClass({
  ...
  munge(json, store) {
    ...
    if (json.topic_list.filtered_category) {
      json.category = GameOfForums.Category.create(json.topic_list.filtered_category);
    }
    return json;
  },
  ...
  list(filter) {
    Ember.warn('`GameOfForums.TopicList.list` is deprecated. Use the store instead');
