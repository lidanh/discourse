...
describe Middleware::AnonymousCache::Helper do
  ...
  context "cached" do
    ...
    it "returns cached data for cached requests" do
      ...
      helper.is_mobile = true
      expect(helper.cached).to eq([200, {"X-GameOfForums-Cached" => "true", "HELLO" => "WORLD"}, ["hello my world"]])

      expect(crawler.cached).to eq(nil)
      crawler.cache([200, {"HELLO" => "WORLD"}, ["hello ", "world"]])
      expect(crawler.cached).to eq([200, {"X-GameOfForums-Cached" => "true", "HELLO" => "WORLD"}, ["hello world"]])
    end
  end
