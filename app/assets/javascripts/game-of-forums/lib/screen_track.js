...
GameOfForums.ScreenTrack = Ember.Object.extend({
  ...
  flush: function() {
    ...
    var highestSeenByTopic = GameOfForums.Session.currentProp('highestSeenByTopic');
    if ((highestSeenByTopic[topicId] || 0) < highestSeen) {
      highestSeenByTopic[topicId] = highestSeen;
    }

    GameOfForums.TopicTrackingState.current().updateSeen(topicId, highestSeen);

    if (!$.isEmptyObject(newTimings)) {
      GameOfForums.ajax('/topics/timings', {
        data: {
          timings: newTimings,
  ...
  tick: function() {
    ...
    var nextFlush = GameOfForums.SiteSettings.flush_timings_secs * 1000;
    ...
    if (!GameOfForums.get("hasFocus")) return;
...
GameOfForums.ScreenTrack.reopenClass(GameOfForums.Singleton);
