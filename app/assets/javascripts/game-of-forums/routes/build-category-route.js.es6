import { queryParams, filterQueryParams, findTopicList } from 'game-of-forums/routes/build-topic-route';

// A helper function to create a category route with parameters
export default function(filter, params) {
  return GameOfForums.Route.extend({
    queryParams: queryParams,

    model: function(modelParams) {
      return GameOfForums.Category.findBySlug(modelParams.slug, modelParams.parentSlug);
    },
    ...
    _setupNavigation: function(model) {
      var noSubcategories = params && !!params.no_subcategories,
          filterMode = "c/" + GameOfForums.Category.slugFor(model) + (noSubcategories ? "/none" : "") + "/l/" + filter;
    ...
    _createSubcategoryList: function(model) {
      ...
      if (Em.isNone(model.get('parentCategory')) && GameOfForums.SiteSettings.show_subcategory_list) {
        var self = this;
        return GameOfForums.CategoryList.listForParent(model).then(function(list) {
    ...
    _retrieveTopicList: function(model, transition) {
      var listFilter = "c/" + GameOfForums.Category.slugFor(model) + "/l/" + filter,
          self = this;
      ...
      return findTopicList(this.store, listFilter, findOpts, extras).then(function(list) {
        GameOfForums.TopicList.hideUniformCategory(list, model);
