...
const TopicRoute = GameOfForums.Route.extend(ShowFooter, {
  ...
  actions: {
    ...
    postChangedRoute(currentPost) {
      ...
      if (isTransitioning || GameOfForums.TopicRoute.disableReplaceState) { return; }
  ...
  _replaceUnlessScrolling(url) {
    ...
    if (currentPos === lastScrollPos) {
      GameOfForums.URL.replaceState(url);
      return;
    }
  ...
  deactivate() {
    ...
    GameOfForums.ScreenTrack.current().stop();
  ...
  setupController(controller, model) {
    ...
    GameOfForums.TopicRoute.trigger('setupTopicController', this);
    ...
    GameOfForums.TopicTrackingState.current().trackIncoming('all');
    controller.subscribe();

    this.controllerFor('topic-progress').set('model', model);
    // We reset screen tracking every time a topic is entered
    GameOfForums.ScreenTrack.current().start(model.get('id'), controller);
  }
