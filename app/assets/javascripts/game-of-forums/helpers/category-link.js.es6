...
import { iconHTML } from 'game-of-forums/helpers/fa-icon';
...
export function categoryBadgeHTML(category, opts) {
  ...
  if ((!category) ||
        (!opts.allowUncategorized &&
         Em.get(category, 'id') === GameOfForums.Site.currentProp("uncategorized_category_id") &&
         GameOfForums.SiteSettings.suppress_uncategorized_badge
        )
     ) return "";

  var description = get(category, 'description_text'),
      restricted = get(category, 'read_restricted'),
      url = GameOfForums.getURL("/c/") + GameOfForums.Category.slugFor(category),
  ...
  if (!opts.hideParent) {
    parentCat = GameOfForums.Category.findById(get(category, 'parent_category_id'));
  }
  ...
  extraClasses = GameOfForums.SiteSettings.category_style ? GameOfForums.SiteSettings.category_style + extraClasses : extraClasses;
