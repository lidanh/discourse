...
groups.forEach(function(group){
  group.icons = _.reject(group.icons, function(obj){
    return !GameOfForums.Emoji.exists(obj);
  });
});

// export so others can modify
GameOfForums.Emoji.groups = groups;
...
var initializeUngroupedIcons = function(){
  ...
  var emojis = GameOfForums.Emoji.list();
...
var toolbar = function(selected){
  ...
  return _.map(groups, function(g, i){
    ...
    var row = {src: GameOfForums.Emoji.urlFor(icon), groupId: i};
    if(i === selected){
      row.selected = true;
    }
    return row;
...
var bindEvents = function(page,offset){
  var composerController = GameOfForums.__container__.lookup('controller:composer');
  ...
  $('.emoji-page a').click(function(){
    ...
    var title = $(this).attr('title');
    var html = "<img src='" + GameOfForums.Emoji.urlFor(title) + "' class='emoji'> <span>:" + title + ":<span>";
...
var render = function(page, offset){
  ...
  for(var i=offset; i<max; i++){
    ...
    row.push({src: GameOfForums.Emoji.urlFor(icons[i]), title: icons[i]});
  }
  rows.push(row);
