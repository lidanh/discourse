const Site = GameOfForums.Model.extend({
...
Site.reopenClass(GameOfForums.Singleton, {

  // The current singleton will retrieve its attributes from the `PreloadStore`.
  createCurrent() {
    return GameOfForums.Site.create(PreloadStore.get('site'));
  },
  ...
  create() {
    ...
    if (result.categories) {
      ...
      result.categories = _.map(result.categories, function(c) {
        return result.categoriesById[c.id] = GameOfForums.Category.create(c);
      });
    ...
    if (result.trust_levels) {
      result.trustLevels = result.trust_levels.map(function (tl) {
        return GameOfForums.TrustLevel.create(tl);
      });
    ...
    if (result.post_action_types) {
      ...
      result.post_action_types = _.map(result.post_action_types,function(p) {
        const actionType = GameOfForums.PostActionType.create(p);
    ...
    if (result.archetypes) {
      result.archetypes = _.map(result.archetypes,function(a) {
        return GameOfForums.Archetype.create(a);
      });
    }
