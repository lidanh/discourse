...
relativeAgeTiny = function(date){
  ...
  case(distanceInMinutes >= 90 && distanceInMinutes <= 1439):
    ...
    break;
  case(GameOfForums.SiteSettings.relative_date_duration === 0 && distanceInMinutes <= 525599):
    formatted = shortDateNoYear(date);
    break;
  case(distanceInMinutes >= 1440 && distanceInMinutes <= 2519):
    formatted = t("x_days", {count: 1});
    break;
  case(distanceInMinutes >= 2520 && distanceInMinutes <= ((GameOfForums.SiteSettings.relative_date_duration||14) * 1440)):
