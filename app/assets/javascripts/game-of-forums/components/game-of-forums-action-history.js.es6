  ...
  renderString(buffer) {
      ...
      this.get('actionsHistory').forEach(function(c) {
        ...
        if (c.get('usersExpanded')) {
          ...
          c.get('users').forEach(function(u) {
            iconsHtml += "<a href=\"" + GameOfForums.getURL("/users/") + u.get('username_lower') + "\" data-user-card=\"" + u.get('username_lower') + "\">";
            if (u.post_url) {
              postUrl = postUrl || u.post_url;
            }
            iconsHtml += GameOfForums.Utilities.avatarImg({
              size: 'small',
    ...
    if (post.get('deleted')) {
      buffer.push("<div class='post-action'>" +
                  ...
                  GameOfForums.Utilities.tinyAvatar(post.get('postDeletedBy.avatar_template'), {title: post.get('postDeletedBy.username')}) +
                  GameOfForums.Formatter.autoUpdatingRelativeAge(new Date(post.get('postDeletedAt'))) +
                  "</div>");
    }
  },
