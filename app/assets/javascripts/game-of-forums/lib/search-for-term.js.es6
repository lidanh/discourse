...
function searchForTerm(term, opts) {
  ...
  return GameOfForums.ajax('/search/query', { data: data }).then(function(results){
    ...
    results.posts = results.posts.map(function(post){
      post = GameOfForums.Post.create(post);
      post.set('topic', topicMap[post.topic_id]);
      return post;
    });

    results.users = results.users.map(function(user){
      user = GameOfForums.User.create(user);
      return user;
    });

    results.categories = results.categories.map(function(category){
      return GameOfForums.Category.list().findProperty('id', category.id);
    }).compact();
