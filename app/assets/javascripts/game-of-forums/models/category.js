GameOfForums.Category = GameOfForums.Model.extend({
  ...
  init: function() {
    ...
    this.set("permissions", Em.A(_.map(this.group_permissions, function(elem){
      ...
      return {
                ...
                permission: GameOfForums.PermissionType.create({id: elem.permission_type})
      };
    })));
  },

  availablePermissions: function(){
    return [  GameOfForums.PermissionType.create({id: GameOfForums.PermissionType.FULL}),
              GameOfForums.PermissionType.create({id: GameOfForums.PermissionType.CREATE_POST}),
              GameOfForums.PermissionType.create({id: GameOfForums.PermissionType.READONLY})
           ];
  }.property(),
  ...
  url: function() {
    return GameOfForums.getURL("/c/") + GameOfForums.Category.slugFor(this);
  }.property('name'),
  ...
  moreTopics: function() {
    return this.get('topic_count') > GameOfForums.SiteSettings.category_featured_topics;
  ...
  destroy: function() {
    return GameOfForums.ajax("/categories/" + (this.get('id') || this.get('slug')), { type: 'DELETE' });
  },
  ...
  permissions: function(){
    return Em.A([
      {group_name: "everyone", permission: GameOfForums.PermissionType.create({id: 1})},
      {group_name: "admins", permission: GameOfForums.PermissionType.create({id: 2}) },
      {group_name: "crap", permission: GameOfForums.PermissionType.create({id: 3}) }
    ]);
  }.property(),
  ...
  featuredTopics: function() {
    var topics = this.get('topics');
    if (topics && topics.length) {
      return topics.slice(0, GameOfForums.SiteSettings.category_featured_topics || 2);
    }
  }.property('topics'),
  ...
  isUncategorizedCategory: function() {
    return this.get('id') === GameOfForums.Site.currentProp("uncategorized_category_id");
  }.property('id')
});

var _uncategorized;

GameOfForums.Category.reopenClass({

  findUncategorized: function() {
    _uncategorized = _uncategorized || GameOfForums.Category.list().findBy('id', GameOfForums.Site.currentProp('uncategorized_category_id'));
  ...
  slugFor: function(category) {
    ...
    if (parentCategory) {
      result = GameOfForums.Category.slugFor(parentCategory) + "/";
    }
  ...
  list: function() {
    if (GameOfForums.SiteSettings.fixed_category_positions) {
      return GameOfForums.Site.currentProp('categories');
    } else {
      return GameOfForums.Site.currentProp('sortedCategories');
    }
  },
  ...
  idMap: function() {
    return GameOfForums.Site.currentProp('categoriesById');
  },

  findSingleBySlug: function(slug) {
    return GameOfForums.Category.list().find(function(c) {
      return GameOfForums.Category.slugFor(c) === slug;
    });
  },

  findById: function(id) {
    if (!id) { return; }
    return GameOfForums.Category.idMap()[id];
  },

  findByIds: function(ids){
    var categories = [];
    _.each(ids, function(id){
      var found = GameOfForums.Category.findById(id);
      if(found){
  ...
  findBySlug: function(slug, parentSlug) {
    var categories = GameOfForums.Category.list(),
        category;

    if (parentSlug) {
      var parentCategory = GameOfForums.Category.findSingleBySlug(parentSlug);
      if (parentCategory) {
        if (slug === 'none') { return parentCategory; }

        category = categories.find(function(item) {
          return item && item.get('parentCategory') === parentCategory && GameOfForums.Category.slugFor(item) === (parentSlug + "/" + slug);
        });
      }
    } else {
      category = GameOfForums.Category.findSingleBySlug(slug);
  ...
  reloadById: function(id) {
    return GameOfForums.ajax("/c/" + id + "/show.json").then(function (result) {
      return GameOfForums.Category.create(result.category);
    });
  }
});
