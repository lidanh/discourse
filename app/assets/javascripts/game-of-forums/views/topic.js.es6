...
import AddArchetypeClass from 'game-of-forums/mixins/add-archetype-class';
import ClickTrack from 'game-of-forums/lib/click-track';
import { listenForViewEvent } from 'game-of-forums/lib/app-events';
import { categoryBadgeHTML } from 'game-of-forums/helpers/category-link';

const TopicView = GameOfForums.View.extend(AddCategoryClass, AddArchetypeClass, GameOfForums.Scrolling, {
  templateName: 'topic',
  ...
  _composeChanged: function() {
    const composerController = GameOfForums.get('router.composerController');
  ...
  _inserted: function() {
    ...
    this.$().on('mouseup.discourse-redirect', '.cooked a, a.track-link', function(e) {
      ...
      if (selection.type === "Range" || selection.rangeCount > 0) {
        if (GameOfForums.Utilities.selectedText() !== "") {
          return true;
        }
      }
  ...
  gotFocus: function() {
    if (GameOfForums.get('hasFocus')){
      this.scrolled();
    }
  }.observes("GameOfForums.hasFocus"),
  ...
  browseMoreMessage: function() {
    var opts = { latestLink: "<a href=\"" + GameOfForums.getURL("/latest") + "\">" + I18n.t("topic.view_latest_topics") + "</a>" },
        category = this.get('controller.content.category');

    if(category && Em.get(category, 'id') === GameOfForums.Site.currentProp("uncategorized_category_id")) {
      category = null;
    }
    ...
    if (category) {
      ...
      opts.catLink = "<a href=\"" + GameOfForums.getURL("/categories") + "\">" + I18n.t("topic.browse_all_categories") + "</a>";
    }
