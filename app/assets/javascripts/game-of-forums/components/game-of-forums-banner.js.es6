import VisibleComponent from "game-of-forums/components/visible";
  ...
  visible: function () {
    var bannerKey = this.get("banner.key"),
        dismissedBannerKey = this.get("user.dismissed_banner_key") ||
                             GameOfForums.KeyValueStore.get("dismissed_banner_key");
  ...
  actions: {
    dismiss: function () {
      if (this.get("user")) {
        ...
        GameOfForums.KeyValueStore.set({ key: "dismissed_banner_key", value: this.get("banner.key") });
      }
    }
  },
