...
export default ObjectController.extend(CanCheckEmails, {
  ...
  viewingSelf: function() {
    return this.get('content.username') === GameOfForums.User.currentProp('username');
  ...
  showBadges: function() {
    return GameOfForums.SiteSettings.enable_badges && (this.get('content.badge_count') > 0);
  }.property('content.badge_count'),

  privateMessageView: function() {
    return (this.get('userActionType') === GameOfForums.UserAction.TYPES.messages_sent) ||
           (this.get('userActionType') === GameOfForums.UserAction.TYPES.messages_received);
  ...
  actions: {
    adminDelete: function() {
      GameOfForums.AdminUser.find(this.get('username').toLowerCase()).then(function(user){
    ...
    exportUserArchive: function() {
      bootbox.confirm(
        ...
        function(confirmed) {
          if (confirmed) {
            GameOfForums.ExportCsv.exportUserArchive();
          }
        }
      );
    }
