/**
  ...
  respect GameOfForums paths and the run loop.
**/
var _trackView = false;

GameOfForums.Ajax = Em.Mixin.create({
  ...
  **/
  ajax: function() {
    ...
    if (args.success) {
      throw "GameOfForums.ajax should use promises, received 'success' callback";
    }
    if (args.error) {
      throw "DEPRECATION: GameOfForums.ajax should use promises, received 'error' callback";
    }
    ...
    var performAjax = function(resolve, reject) {
      ...
      if (_trackView && (!args.type || args.type === "GET")) {
        _trackView = false;
        args.headers['GameOfForums-Track-View'] = true;
      }

      args.success = function(data, textStatus, xhr) {
        if (xhr.getResponseHeader('GameOfForums-Readonly')) {
          Ember.run(function() {
            GameOfForums.Site.currentProp('isReadOnly', true);
          });
        }
      ...
      args.error = function(xhr, textStatus, errorThrown) {
        ...
        if (xhr.status === 403 && xhr.responseText === "['BAD CSRF']") {
          GameOfForums.Session.current().set('csrfToken', null);
        }
      ...
      if (args.dataType === "script") {
        args.headers['GameOfForums-Script'] = true;
      }
      ...
      $.ajax(GameOfForums.getURL(url), args);
    };
    ...
    if(args.type && args.type.toUpperCase() !== 'GET' && !GameOfForums.Session.currentProp('csrfToken')){
      return new Ember.RSVP.Promise(function(resolve, reject){
        $.ajax(GameOfForums.getURL('/session/csrf'), {cache: false})
           .success(function(result){
              GameOfForums.Session.currentProp('csrfToken', result.csrf);
