GameOfForums.Utilities = {
  ...
  **/
  ...
  avatarUrl: function(template, size) {
    if (!template) { return ""; }
    var rawSize = GameOfForums.Utilities.getRawSize(GameOfForums.Utilities.translateSize(size));
  ...
  avatarImg: function(options) {
    var size = GameOfForums.Utilities.translateSize(options.size);
    var url = GameOfForums.Utilities.avatarUrl(options.avatarTemplate, size);
  ...
  tinyAvatar: function(avatarTemplate, options) {
    return GameOfForums.Utilities.avatarImg(_.merge({avatarTemplate: avatarTemplate, size: 'tiny' }, options));
  },

  postUrl: function(slug, topicId, postNumber) {
    var url = GameOfForums.getURL("/t/");
    if (slug) {
      url += slug + "/";
    } else {
      url += 'topic/';
  ...
  userUrl: function(username) {
    return GameOfForums.getURL("/users/" + username.toLowerCase());
  },
  ...
  validateUploadedFiles: function(files, bypassNewUserRestriction) {
    ...
    var type = GameOfForums.Utilities.isAnImage(upload.name) ? 'image' : 'attachment';

    return GameOfForums.Utilities.validateUploadedFile(upload, type, bypassNewUserRestriction);
  },

  validateUploadedFile: function(file, type, bypassNewUserRestriction) {
    // check that the uploaded file is authorized
    if (!GameOfForums.Utilities.authorizesAllExtensions() &&
        !GameOfForums.Utilities.isAuthorizedUpload(file)) {
      var extensions = GameOfForums.Utilities.authorizedExtensions();
    ...
    if (!bypassNewUserRestriction) {
      ...
      if (!GameOfForums.User.current().isAllowedToUploadAFile(type)) {
    ...
    var maxSizeKB = GameOfForums.SiteSettings['max_' + type + '_size_kb'];
  ...
  authorizesAllExtensions: function() {
    return GameOfForums.SiteSettings.authorized_extensions.indexOf("*") >= 0;
  },

  isAuthorizedUpload: function(file) {
    if (file && file.name) {
      var extensions = _.chain(GameOfForums.SiteSettings.authorized_extensions.split("|"))
  ...
  authorizedExtensions: function() {
    return _.chain(GameOfForums.SiteSettings.authorized_extensions.split("|"))
            .reject(function(extension) { return extension.indexOf("*") >= 0; })
  ...
  getUploadMarkdown: function(upload) {
    if (GameOfForums.Utilities.isAnImage(upload.original_filename)) {
  ...
  allowsImages: function() {
    return GameOfForums.Utilities.authorizesAllExtensions() ||
           (/(png|jpe?g|gif|bmp|tiff?|svg|webp)/i).test(GameOfForums.Utilities.authorizedExtensions());
  },

  allowsAttachments: function() {
    return GameOfForums.Utilities.authorizesAllExtensions() ||
           !(/((png|jpe?g|gif|bmp|tiff?|svg|webp)(,\s)?)+$/i).test(GameOfForums.Utilities.authorizedExtensions());
  },
  ...
  displayErrorForUpload: function(data) {
    ...
    if (data.jqXHR) {
      switch (data.jqXHR.status) {
        ...
        case 413:
          var maxSizeKB = GameOfForums.SiteSettings.max_image_size_kb;
          bootbox.alert(I18n.t('post.errors.file_too_large', { max_size_kb: maxSizeKB }));
          return;
  ...
  defaultHomepage: function() {
    ...
    return GameOfForums.SiteSettings.top_menu.split("|")[0].split(",")[0];
  }
