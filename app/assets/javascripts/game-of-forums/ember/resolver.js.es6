...
export default Ember.DefaultResolver.extend({
  ...
  normalize(fullName) {
    ...
    if (split.length > 1) {
      var gameOfForumsBase = 'game-of-forums/' + split[0] + 's/';
      ...
      if (requirejs.entries[gameOfForumsBase + dashed] || requirejs.entries[adminBase + dashed]) {
  ...
  findLoadingView: loadingResolver(function() {
    if (!_loadingView) {
      _loadingView = require('game-of-forums/views/loading', null, null, true /* force sync */);
