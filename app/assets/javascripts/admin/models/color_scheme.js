/**
  ...
  @module GameOfForums
**/
GameOfForums.ColorScheme = GameOfForums.Model.extend(Ember.Copyable, {
  ...
  copy: function() {
    var newScheme = GameOfForums.ColorScheme.create({name: this.get('name'), enabled: false, can_edit: true, colors: Em.A()});
    _.each(this.get('colors'), function(c){
      newScheme.colors.pushObject(GameOfForums.ColorSchemeColor.create({name: c.get('name'), hex: c.get('hex'), default_hex: c.get('default_hex')}));
    });
    return newScheme;
  },
  ...
  save: function(opts) {
    ...
    return GameOfForums.ajax("/admin/color_schemes" + (this.id ? '/' + this.id : '') + '.json', {
  ...
  destroy: function() {
    if (this.id) {
      return GameOfForums.ajax("/admin/color_schemes/" + this.id, { type: 'DELETE' });
    }
  }
...
GameOfForums.ColorScheme.reopenClass({
  findAll: function() {
    var colorSchemes = ColorSchemes.create({ content: [], loading: true });
    GameOfForums.ajax('/admin/color_schemes').then(function(all) {
      _.each(all, function(colorScheme){
        colorSchemes.pushObject(GameOfForums.ColorScheme.create({
          id: colorScheme.id,
          name: colorScheme.name,
          enabled: colorScheme.enabled,
          is_base: colorScheme.is_base,
          colors: colorScheme.colors.map(function(c) { return GameOfForums.ColorSchemeColor.create({name: c.name, hex: c.hex, default_hex: c.default_hex}); })
        }));
      });
