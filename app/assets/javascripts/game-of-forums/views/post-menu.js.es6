...
var PostMenuView = GameOfForums.View.extend(StringBuffer, {
  tagName: 'section',
  ...
  renderButtons: function(post, buffer) {
    ...
    if (typeof hiddenButtons === "undefined") {
      if (!Em.isEmpty(GameOfForums.SiteSettings.post_menu_hidden_items)) {
        hiddenButtons = GameOfForums.SiteSettings.post_menu_hidden_items.split('|');
      } else {
        hiddenButtons = [];
      }
    }
    ...
    GameOfForums.SiteSettings.post_menu.split("|").forEach(function(i) {
  ...
  buttonForShare: function(post) {
    if (!GameOfForums.User.current()) return;
    var options = {
  ...
  renderAdminPopup: function(post, buffer) {
    if (!GameOfForums.User.currentProp('canManageTopic')) { return; }
    ...
    var isModerator = post.get('post_type') === GameOfForums.Site.currentProp('post_types.moderator_action'),
    ...
    var html = '<div class="post-admin-menu">' +
                 ...
                 '<ul>' +
                   ...
                   (GameOfForums.User.currentProp('staff') ? '<li class="btn btn-admin" data-action="togglePostType">' + postTypeIcon + postTypeText + '</li>' : '') +
