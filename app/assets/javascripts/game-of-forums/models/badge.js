/**
  A data model representing a badge on GameOfForums
  ...
  @module GameOfForums
**/
GameOfForums.Badge = GameOfForums.Model.extend({
  /**
    Is this a new badge?
  ...
  **/
  ...
  /**
    ...
    @method save
    @returns {Promise} A promise that resolves to the updated `GameOfForums.Badge`
  **/
  save: function(data) {
  ...
  **/
  destroy: function() {
    ...
    return GameOfForums.ajax("/admin/badges/" + this.get('id'), {
      type: "DELETE"
    });
  }
});

GameOfForums.Badge.reopenClass({
  /**
    Create `GameOfForums.Badge` instances from the server JSON response.

    @method createFromJson
    @param {Object} json The JSON returned by the server
    @returns Array or instance of `GameOfForums.Badge` depending on the input JSON
  **/
  createFromJson: function(json) {
    ...
    if ('badge_groupings' in json) {
      json.badge_groupings.forEach(function(badgeGroupingJson) {
        badgeGroupings[badgeGroupingJson.id] = GameOfForums.BadgeGrouping.create(badgeGroupingJson);
      });
    }
    ...
    badges = badges.map(function(badgeJson) {
      var badge = GameOfForums.Badge.create(badgeJson);
  ...
  /**
    Find all `GameOfForums.Badge` instances that have been defined.

    @method findAll
    @returns {Promise} a promise that resolves to an array of `GameOfForums.Badge`
  **/
  findAll: function(opts) {
    ...
    return GameOfForums.ajax('/badges.json' + listable).then(function(badgesJson) {
      return GameOfForums.Badge.createFromJson(badgesJson);
    });
  },

  /**
    Returns a `GameOfForums.Badge` that has the given ID.

    @method findById
    @param {Number} id ID of the badge
    @returns {Promise} a promise that resolves to a `GameOfForums.Badge`
  **/
  findById: function(id) {
    return GameOfForums.ajax("/badges/" + id).then(function(badgeJson) {
      return GameOfForums.Badge.createFromJson(badgeJson);
    });
  }
});
