...
GameOfForums.UserAction = GameOfForums.Model.extend({
  ...
  **/
  descriptionKey: function() {
    ...
    if (action === null || GameOfForums.UserAction.TO_SHOW.indexOf(action) >= 0) {
      if (this.get('isPM')) {
  ...
  targetUser: function() {
    return this.get('target_username') === GameOfForums.User.currentProp('username');
  ...
  targetUserUrl: GameOfForums.computed.url('target_username', '/users/%@'),

  usernameLower: function() {
    return this.get('username').toLowerCase();
  }.property('username'),

  userUrl: GameOfForums.computed.url('usernameLower', '/users/%@'),

  postUrl: function() {
    return GameOfForums.Utilities.postUrl(this.get('slug'), this.get('topic_id'), this.get('post_number'));
  }.property(),

  replyUrl: function() {
    return GameOfForums.Utilities.postUrl(this.get('slug'), this.get('topic_id'), this.get('reply_to_post_number'));
  }.property(),
  ...
  addChild: function(action) {
    ...
    if (!groups) {
      groups = {
        likes: GameOfForums.UserActionGroup.create({ icon: "fa fa-heart" }),
        stars: GameOfForums.UserActionGroup.create({ icon: "fa fa-star" }),
        edits: GameOfForums.UserActionGroup.create({ icon: "fa fa-pencil" }),
        bookmarks: GameOfForums.UserActionGroup.create({ icon: "fa fa-bookmark" })
      };
    }
...
GameOfForums.UserAction.reopenClass({
  collapseStream: function(stream) {
    ...
    stream.forEach(function(item) {
      ...
      if (found === void 0) {
        ...
        var current;
        if (GameOfForums.UserAction.TO_COLLAPSE.indexOf(item.action_type) >= 0) {
          current = GameOfForums.UserAction.create(item);
          item.switchToActing();
