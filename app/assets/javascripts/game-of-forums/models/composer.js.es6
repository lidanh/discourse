...
import { throwAjaxError } from 'game-of-forums/lib/ajax-error';
...
const Composer = RestModel.extend({
  ...
  actionTitle: function() {
    ...
    if (post) {
      postDescription = I18n.t('post.' +  this.get('action'), {
        link: postLink,
        replyAvatar: GameOfForums.Utilities.tinyAvatar(post.get('avatar_template')),
      ...
      if (!GameOfForums.Mobile.mobileView) {
        ...
        if (replyUsername && replyAvatarTemplate && this.get('action') === EDIT) {
          postDescription += " <i class='fa fa-mail-forward reply-to-glyph'></i> " + GameOfForums.Utilities.tinyAvatar(replyAvatarTemplate) + " " + replyUsername;
        }
      }
    }
  ...
  **/
  replyLength: function() {
    ...
    while (GameOfForums.Quote.REGEXP.test(reply)) { reply = reply.replace(GameOfForums.Quote.REGEXP, ""); }
    return reply.replace(/\s+/img, " ").trim().length;
  }.property('reply'),

  _setupComposer: function() {
    const val = (GameOfForums.Mobile.mobileView ? false : (GameOfForums.KeyValueStore.get('composer.showPreview') || 'true'));
  ...
  **/
  ...
  togglePreview() {
    ...
    GameOfForums.KeyValueStore.set({ key: 'composer.showPreview', value: this.get('showPreview') });
  },
  ...
  **/
  ...
  createPost(opts) {
    ...
    return createdPost.save().then(function(result) {
      ...
      if (topic) {
        ...
        GameOfForums.notifyPropertyChange('globalNotice');
      }
  ...
  saveDraft() {
    ...
    return GameOfForums.Draft.save(this.get('draftKey'), this.get('draftSequence'), data)
      .then(function() {
...
Composer.reopenClass({
  ...
  loadDraft(opts) {
    ...
    try {
      ...
      draft = null;
      GameOfForums.Draft.clear(draftKey, draftSequence);
    }
  ...
  create(args) {
    args = args || {};
    args.user = args.user || GameOfForums.User.current();
    args.site = args.site || GameOfForums.Site.current();
    args.siteSettings = args.siteSettings || GameOfForums.SiteSettings;
