/**
  A data model representing a user badge grant on GameOfForums
  ...
  @module GameOfForums
**/
GameOfForums.UserBadge = GameOfForums.Model.extend({
  postUrl: function() {
  ...
  **/
  revoke: function() {
    return GameOfForums.ajax("/user_badges/" + this.get('id'), {
      type: "DELETE"
    });
  }
});

GameOfForums.UserBadge.reopenClass({
  /**
    Create `GameOfForums.UserBadge` instances from the server JSON response.

    @method createFromJson
    @param {Object} json The JSON returned by the server
    @returns Array or instance of `GameOfForums.UserBadge` depending on the input JSON
  **/
  createFromJson: function(json) {
    ...
    json.users.forEach(function(userJson) {
      users[userJson.id] = GameOfForums.User.create(userJson);
    });
    ...
    json.topics.forEach(function(topicJson) {
      topics[topicJson.id] = GameOfForums.Topic.create(topicJson);
    });

    // Create the badges.
    if (json.badges === undefined) { json.badges = []; }
    var badges = {};
    GameOfForums.Badge.createFromJson(json).forEach(function(badge) {
    ...
    userBadges = userBadges.map(function(userBadgeJson) {
      var userBadge = GameOfForums.UserBadge.create(userBadgeJson);
  ...
  /**
    ...
    @returns {Promise} a promise that resolves to an array of `GameOfForums.UserBadge`.
  **/
  findByUsername: function(username, options) {
    ...
    return GameOfForums.ajax(url).then(function(json) {
      return GameOfForums.UserBadge.createFromJson(json);
    });
  },
  ...
  **/
  findByBadgeId: function(badgeId, options) {
    ...
    return GameOfForums.ajax("/user_badges.json", {
      data: options
    }).then(function(json) {
      return GameOfForums.UserBadge.createFromJson(json);
    });
  ...
  /**
    ...
    @returns {Promise} a promise that resolves to an instance of `GameOfForums.UserBadge`.
  **/
  grant: function(badgeId, username, reason) {
    return GameOfForums.ajax("/user_badges", {
      type: "POST",
      data: {
        username: username,
        badge_id: badgeId,
