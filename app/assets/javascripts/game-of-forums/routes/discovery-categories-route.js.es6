...
GameOfForums.DiscoveryCategoriesRoute = GameOfForums.Route.extend(OpenComposer, ShowFooter, {
  renderTemplate() {
  ...
  model() {
    ...
    return GameOfForums.CategoryList.list('categories').then(function(list) {
      const tracking = GameOfForums.TopicTrackingState.current();
      if (tracking) {
  ...
  titleToken() {
    if (GameOfForums.Utilities.defaultHomepage() === "categories") { return; }
  ...
  actions: {
    createCategory() {
      ...
      const model = GameOfForums.Category.create({
...
export default GameOfForums.DiscoveryCategoriesRoute;
