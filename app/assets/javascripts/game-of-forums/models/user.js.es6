...
const User = RestModel.extend({
  ...
  /**
    ...
    @property stream
    @type {GameOfForums.UserStream}
  **/
  stream: function() {
    return GameOfForums.UserStream.create({ user: this });
  }.property(),
  ...
  /**
    ...
    @type {GameOfForums.UserPostsStream}
  **/
  postsStream: function() {
    return GameOfForums.UserPostsStream.create({ user: this });
  }.property(),
  ...
  **/
  displayName: function() {
    if (GameOfForums.SiteSettings.enable_names && !this.blank('name')) {
  ...
  **/
  profileBackground: function() {
    ...
    if (Em.isEmpty(url) || !GameOfForums.SiteSettings.allow_profile_backgrounds) { return; }
    return ('background-image: url(' + GameOfForums.getURLWithCDN(url) + ')').htmlSafe();
  ...
  **/
  path: function(){
    return GameOfForums.getURL('/users/' + this.get('username_lower'));
  ...
  /**
    ...
    @type {String}
  **/
  adminPath: GameOfForums.computed.url('username_lower', "/admin/users/%@"),
  ...
  **/
  trustLevel: function() {
    return GameOfForums.Site.currentProp('trustLevels').findProperty('id', parseInt(this.get('trust_level'), 10));
  ...
  suspendedTillDate: function() {
    return GameOfForums.Formatter.longDate(this.get('suspended_till'));
  }.property('suspended_till'),
  ...
  **/
  changeUsername: function(newUsername) {
    return GameOfForums.ajax("/users/" + this.get('username_lower') + "/preferences/username", {
      type: 'PUT',
  ...
  **/
  changeEmail: function(email) {
    return GameOfForums.ajax("/users/" + this.get('username_lower') + "/preferences/email", {
      type: 'PUT',
      data: { email: email }
    });
  },
  ...
  **/
  copy: function() {
    return GameOfForums.User.create(this.getProperties(Ember.keys(this)));
  },
  ...
  **/
  save: function() {
    ...
    if (!GameOfForums.SiteSettings.edit_history_visible_to_public) {
      data['edit_history_public'] = this.get('edit_history_public');
    }

    // TODO: We can remove this when migrated fully to rest model.
    this.set('isSaving', true);
    return GameOfForums.ajax("/users/" + this.get('username_lower'), {
      data: data,
      type: 'PUT'
    }).then(function(data) {
      ...
      GameOfForums.User.current().setProperties(userProps);
    }).finally(() => {
  ...
  **/
  changePassword: function() {
    return GameOfForums.ajax("/session/forgot_password", {
      dataType: 'json',
  ...
  **/
  loadUserAction: function(id) {
    ...
    return GameOfForums.ajax("/user_actions/" + id + ".json", { cache: 'false' }).then(function(result) {
      if (result && result.user_action) {
        ...
        var action = GameOfForums.UserAction.collapseStream([GameOfForums.UserAction.create(ua)]);
  ...
  inAllStream: function(ua) {
    return ua.action_type === GameOfForums.UserAction.TYPES.posts ||
           ua.action_type === GameOfForums.UserAction.TYPES.topics;
  },
  ...
  findDetails: function(options) {
    ...
    return PreloadStore.getAndRemove("user_" + user.get('username'), function() {
      return GameOfForums.ajax("/users/" + user.get('username') + '.json', {data: options});
    }).then(function (json) {

      if (!Em.isEmpty(json.user.stats)) {
        json.user.stats = GameOfForums.User.groupStats(_.map(json.user.stats,function(s) {
          if (s.count) s.count = parseInt(s.count, 10);
          return GameOfForums.UserActionStat.create(s);
        }));
      }

      if (!Em.isEmpty(json.user.custom_groups)) {
        json.user.custom_groups = json.user.custom_groups.map(function (g) {
          return GameOfForums.Group.create(g);
        });
      }

      if (json.user.invited_by) {
        json.user.invited_by = GameOfForums.User.create(json.user.invited_by);
      }

      if (!Em.isEmpty(json.user.featured_user_badge_ids)) {
        var userBadgesMap = {};
        GameOfForums.UserBadge.createFromJson(json).forEach(function(userBadge) {
      ...
      if (json.user.card_badge) {
        json.user.card_badge = GameOfForums.Badge.create(json.user.card_badge);
      }
  ...
  findStaffInfo: function() {
    if (!GameOfForums.User.currentProp("staff")) { return Ember.RSVP.resolve(null); }
    var self = this;
    return GameOfForums.ajax("/users/" + this.get("username_lower") + "/staff-info.json").then(function(info) {
  ...
  */
  pickAvatar: function(uploadId) {
    ...
    return GameOfForums.ajax("/users/" + this.get("username_lower") + "/preferences/avatar/pick", {
      type: 'PUT',
  ...
  **/
  isAllowedToUploadAFile: function(type) {
    return this.get('staff') ||
           ...
           GameOfForums.SiteSettings['newuser_max_' + type + 's'] > 0;
  },
  ...
  **/
  createInvite: function(email, groupNames) {
    return GameOfForums.ajax('/invites', {
      type: 'POST',
  ...
  updateMutedCategories: function() {
    this.set("mutedCategories", GameOfForums.Category.findByIds(this.muted_category_ids));
  }.observes("muted_category_ids"),

  updateTrackedCategories: function() {
    this.set("trackedCategories", GameOfForums.Category.findByIds(this.tracked_category_ids));
  }.observes("tracked_category_ids"),

  updateWatchedCategories: function() {
    this.set("watchedCategories", GameOfForums.Category.findByIds(this.watched_category_ids));
  ...
  "delete": function() {
    if (this.get('can_delete_account')) {
      return GameOfForums.ajax("/users/" + this.get('username'), {
        type: 'DELETE',
  ...
  dismissBanner: function (bannerKey) {
    ...
    GameOfForums.ajax("/users/" + this.get('username'), {
      type: 'PUT',
  ...
  checkEmail: function () {
    var self = this;
    return GameOfForums.ajax("/users/" + this.get("username_lower") + "/emails.json", {
      type: "PUT",
      data: { context: window.location.pathname }
...
User.reopenClass(GameOfForums.Singleton, {

  // Find a `GameOfForums.User` for a given username.
  findByUsername: function(username, options) {
    const user = GameOfForums.User.create({username: username});
    return user.findDetails(options);
  },

  // TODO: Use app.register and junk GameOfForums.Singleton
  createCurrent: function() {
    var userJson = PreloadStore.get('currentUser');
    if (userJson) {
      const store = GameOfForums.__container__.lookup('store:main');
  ...
  **/
  logout: function() {
    var gameOfForumsUserClass = this;
    return GameOfForums.ajax("/session/" + GameOfForums.User.currentProp('username'), {
      type: 'DELETE'
    }).then(function () {
      gameOfForumsUserClass.currentUser = null;
    });
  },
  ...
  **/
  checkUsername: function(username, email, forUserId) {
    return GameOfForums.ajax('/users/check_username', {
  ...
  **/
  groupStats: function(stats) {
    var responses = GameOfForums.UserActionStat.create({
      count: 0,
      action_type: GameOfForums.UserAction.TYPES.replies
    });
    ...
    result.forEach(function(item, index){
     if(item.action_type === GameOfForums.UserAction.TYPES.topics || item.action_type === GameOfForums.UserAction.TYPES.posts){
       insertAt = index + 1;
     }
    });
  ...
  **/
  createAccount: function(attrs) {
    return GameOfForums.ajax("/users", {
      data: {
