...
export default ObjectController.extend(ModalFunctionality, {
  ...
  submitEnabled: function() {
    ...
    if (selected.get('is_custom_flag')) {
      ...
      return len >= GameOfForums.SiteSettings.min_private_message_post_length &&
             len <= GameOfForums.PostActionType.MAX_MESSAGE_LENGTH;
    }
    return true;
  ...
  canTakeAction: function() {
    ...
    return GameOfForums.User.currentProp('staff');
  ...
  canDeleteSpammer: function() {
    ...
    if (GameOfForums.User.currentProp('staff') && this.get('selected.name_key') === 'spam') {
  ...
  fetchUserDetails: function() {
    if( GameOfForums.User.currentProp('staff') && this.get('model.username') ) {
      const flagController = this;
      GameOfForums.AdminUser.find(this.get('model.username').toLowerCase()).then(function(user){
