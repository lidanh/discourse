import Sharing from 'game-of-forums/lib/sharing';
...
export default {
  ...
  initialize: function() {
    ...
    GameOfForums.ShareLink = {};
    GameOfForums.ShareLink.addTarget = function(id, source) {
      Ember.warn('GameOfForums.ShareLink.addTarget is deprecated. Import `Sharing` and call `addSource` instead.');
      source.id = id;
    ...
    Sharing.addSource({
      ...
      generateUrl: function(link, title) {
        return "mailto:?to=&subject=" + encodeURIComponent('[' + GameOfForums.SiteSettings.title + '] ' + title) + "&body=" + encodeURIComponent(link);
      }
    });
  }
};
