...
export default ObjectController.extend(Presence, {
  ...
  maxLength: GameOfForums.computed.setting('max_username_length'),
  minLength: GameOfForums.computed.setting('min_username_length'),
  ...
  unchanged: GameOfForums.computed.propertyEqual('newUsername', 'username'),
  ...
  checkTaken: function() {
    if( this.get('newUsername') && this.get('newUsername').length < this.get('minLength') ) {
      ...
      GameOfForums.User.checkUsername(this.get('newUsername'), undefined, this.get('content.id')).then(function(result) {
        if (result.errors) {
      ...
      return bootbox.confirm(I18n.t("user.change_username.confirm"), I18n.t("no_value"), I18n.t("yes_value"), function(result) {
        if (result) {
          ...
          self.get('content').changeUsername(self.get('newUsername')).then(function() {
            GameOfForums.URL.redirectTo("/users/" + self.get('newUsername').toLowerCase() + "/preferences");
          }, function() {
            // error
            self.set('error', true);
