...
import OpenComposer from "game-of-forums/mixins/open-composer";
...
const ApplicationRoute = GameOfForums.Route.extend(OpenComposer, {

  siteTitle: GameOfForums.computed.setting('title'),

  actions: {
    _collectTitleTokens(tokens) {
      tokens.push(this.get('siteTitle'));
      GameOfForums.set('_docTitle', tokens.join(' - '));
    },
    ...
    showSearchHelp() {
      ...
      GameOfForums.ajax("/static/search_help.html", { dataType: 'html' }).then(function(model){
    ...
    **/
    ...
    reopenModal() {
      $('#game-of-forums-modal').modal('show');
    },

    editCategory(category) {
      const self = this;
      GameOfForums.Category.reloadById(category.get('id')).then(function (model) {
  ...
  handleShowLogin() {
    if (this.siteSettings.enable_sso) {
      ...
      window.location = GameOfForums.getURL('/session/sso?return_path=' + returnPath);
    } else {
  ...
  _autoLogin(modal, modalClass, notAuto) {
    const methods = Em.get('GameOfForums.LoginMethod.all');
