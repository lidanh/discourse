...
export default function(options) {
  ...
  var completeTerm = function(term) {
    if (term) {
      if (isInput) {
        ...
        me.val(text);
        GameOfForums.Utilities.setCaretPosition(me[0], completeStart + 1 + term.length);
      }
    }
    closeAutocomplete();
  };
  ...
  $(this).on('keypress.autocomplete', function(e) {
    ...
    if (options.key && e.which === options.key.charCodeAt(0)) {
      caretPosition = GameOfForums.Utilities.caretPosition(me[0]);
  ...
  $(this).on('keydown.autocomplete', function(e) {
    ...
    if ((completeStart === null) && e.which === keys.backSpace && options.key) {
      c = GameOfForums.Utilities.caretPosition(me[0]);
      next = me[0].value[c];
      c -= 1;
      initial = c;
      prevIsGood = true;
