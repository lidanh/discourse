...
export default ObjectController.extend(ModalFunctionality, {
  ...
  _changeViewModeOnMobile: function() {
    if (GameOfForums.Mobile.mobileView) { this.set("viewMode", "inline"); }
  }.on("init"),
  ...
  refresh: function(postId, postVersion) {
    ...
    var self = this;
    GameOfForums.Post.loadRevision(postId, postVersion).then(function (result) {
  ...
  hide: function(postId, postVersion) {
    var self = this;
    GameOfForums.Post.hideRevision(postId, postVersion).then(function () {
  ...
  show: function(postId, postVersion) {
    var self = this;
    GameOfForums.Post.showRevision(postId, postVersion).then(function () {
      self.refresh(postId, postVersion);
    });
  },
  ...
  displayShow: function() { return this.get("previous_hidden") && GameOfForums.User.currentProp('staff') && !this.get("loading"); }.property("previous_hidden", "loading"),
  displayHide: function() { return !this.get("previous_hidden") && GameOfForums.User.currentProp('staff') && !this.get("loading"); }.property("previous_hidden", "loading"),
  ...
  previousCategory: function() {
    ...
    if (changes) {
      var category = GameOfForums.Category.findById(changes["previous"]);
  ...
  currentCategory: function() {
    var changes = this.get("category_id_changes");
    if (changes) {
      var category = GameOfForums.Category.findById(changes["current"]);
      return categoryBadgeHTML(category, { allowUncategorized: true });
    }
  }.property("category_id_changes"),
