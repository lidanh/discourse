...
const PostView = GameOfForums.GroupedView.extend(Ember.Evented, {
  ...
  historyHeat: function() {
    ...
    if (updatedAtDate > (rightNow - DAY * GameOfForums.SiteSettings.history_hours_low)) return 'heatmap-high';
    if (updatedAtDate > (rightNow - DAY * GameOfForums.SiteSettings.history_hours_medium)) return 'heatmap-med';
    if (updatedAtDate > (rightNow - DAY * GameOfForums.SiteSettings.history_hours_high)) return 'heatmap-low';
    ...
    if (postNumber) {
      ...
      if (topicId = $aside.data('topic')) {
        ...
        if (topic && topic.get('id') === topicId) {
          ...
          navLink = "<a href='" + GameOfForums.getURL("/t/via-quote/") + topicId + "/" + postNumber + "' title='" + quoteTitle + "' class='quote-other-topic'></a>";
        }
  ...
  _toggleQuote($aside) {
    ...
    if ($aside.data('expanded')) {
      ...
      GameOfForums.ajax("/posts/by_number/" + topicId + "/" + postId).then(function (result) {
      ...
      self.$(".cooked a[href]").each(function() {
        ...
        if (valid) {
          ...
          if ($link.hasClass("track-link") ||
            ...
            $link.append("<span class='badge badge-notification clicks' title='" + I18n.t("topic_map.clicks", {count: lc.clicks}) + "'>" + GameOfForums.Formatter.number(lc.clicks) + "</span>");
          }
        }
      });
    });
  ...
  actions: {
    ...
    toggleReplyHistory(post) {
      ...
      if (GameOfForums.Mobile.mobileView) {
        GameOfForums.URL.routeTo(this.get('post.topic').urlForPostNumber(replyPostNumber));
        return;
      }
      ...
      if(GameOfForums.SiteSettings.experimental_reply_expansion) {
  ...
  _postViewInserted: function() {
    ...
    GameOfForums.ScreenTrack.current().track($post.prop('id'), postNumber);
