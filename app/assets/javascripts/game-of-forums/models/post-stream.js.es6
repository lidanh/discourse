...
const PostStream = RestModel.extend({
  ...
  **/
  refresh(opts) {
    ...
    // Request a topicView
    return GameOfForums.PostStream.loadTopicView(topic.get('id'), opts).then(function (json) {
  ...
  **/
  ...
  triggerRecoveredPost(postId){
    ...
    if(existing){
      ...
      GameOfForums.ajax(url).then(function(p){
  ...
  findReplyHistory(post) {
    const postStream = this,
        url = "/posts/" + post.get('id') + "/reply-history.json?max_replies=" + GameOfForums.SiteSettings.max_reply_history;
  ...
  **/
  ...
  loadIntoIdentityMap(postIds) {
    ...
    const store = this.store;
    return GameOfForums.ajax(url, {data: data}).then(function(result) {
...
PostStream.reopenClass({
  ...
  loadTopicView(topicId, args) {
    ...
    return PreloadStore.getAndRemove("topic_" + topicId, function() {
      return GameOfForums.ajax(url + ".json", {data: opts});
    });
