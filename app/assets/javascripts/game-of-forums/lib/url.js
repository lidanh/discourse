...
GameOfForums.URL = Ember.Object.createWithMixins({
  ...
  replaceState: function(path) {
    if (window.history &&
        ...
        Em.run.next(function() {
          var location = GameOfForums.URL.get('router.location');
  ...
  **/
  routeTo: function(path) {
    ...
    if (GameOfForums.get('requiresRefresh')) {
      document.location.href = GameOfForums.getURL(path);
      return;
    }
    ...
    // handle prefixes
    if (path.match(/^\//)) {
      var rootURL = (GameOfForums.BaseUri === undefined ? "/" : GameOfForums.BaseUri);
    ...
    if (path.indexOf('/my/') === 0) {
      var currentUser = GameOfForums.User.current();
      if (currentUser) {
    ...
    Em.run.scheduleOnce('afterRender', GameOfForums.Route, 'cleanDOM');
  ...
  redirectTo: function(url) {
    window.location = GameOfForums.getURL(url);
  },
  ...
  navigatedToPost: function(oldPath, path) {
    ...
    if (newTopicId) {
      ...
      if (oldTopicId === newTopicId) {
        GameOfForums.URL.replaceState(path);

        var container = GameOfForums.__container__,
        ...
        postStream.refresh(opts).then(function() {
          ...
          GameOfForums.URL.jumpToPost(closest, {skipIfOnScreen: true});
        });
  ...
  **/
  navigatedToHome: function(oldPath, path) {
    var homepage = GameOfForums.Utilities.defaultHomepage();
  ...
  **/
  router: function() {
    return GameOfForums.__container__.lookup('router:main');
  }.property().volatile(),
  ...
  **/
  controllerFor: function(name) {
    return GameOfForums.__container__.lookup('controller:' + name);
  },
