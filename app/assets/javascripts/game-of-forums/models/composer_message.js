/**
  ...
  @module GameOfForums
**/
GameOfForums.ComposerMessage = Em.Object.extend({});

GameOfForums.ComposerMessage.reopenClass({
  /**
    Look for composer messages given the current composing settings.

    @method find
    @param {GameOfForums.Composer} composer The current composer
    @returns {GameOfForums.ComposerMessage} the composer message to display (or null)
  **/
  find: function(composer) {
    ...
    return GameOfForums.ajax('/composer-messages', { data: data }).then(function (messages) {
      return messages.map(function (message) {
        return GameOfForums.ComposerMessage.create(message);
      });
    });
  }
