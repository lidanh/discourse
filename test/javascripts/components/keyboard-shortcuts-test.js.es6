...
module("GameOfForums.KeyboardShortcuts", {
  setup: function() {
    var _bindings = {};
...
var pathBindings = GameOfForums.KeyboardShortcuts.PATH_BINDINGS;

_.each(pathBindings, function(path, binding) {
  var testName = binding + " goes to " + path;

  test(testName, function() {
    GameOfForums.KeyboardShortcuts.bindEvents(testMouseTrap);
    testMouseTrap.trigger(binding);

    ok(GameOfForums.URL.routeTo.calledWith(path));
  });
});

var clickBindings = GameOfForums.KeyboardShortcuts.CLICK_BINDINGS;
...
var functionBindings = GameOfForums.KeyboardShortcuts.FUNCTION_BINDINGS;

_.each(functionBindings, function(func, binding) {
  var testName = binding + " calls " + func;

  test(testName, function() {
    sandbox.stub(GameOfForums.KeyboardShortcuts, func, function() {
...
test("selectDown calls _moveSelection with 1", function() {
  var spy = sandbox.spy(GameOfForums.KeyboardShortcuts, '_moveSelection');

  GameOfForums.KeyboardShortcuts.selectDown();
...
test("selectUp calls _moveSelection with -1", function() {
  ...
  GameOfForums.KeyboardShortcuts.selectUp();
  ok(spy.calledWith(-1), "_moveSelection is called with -1");
});
...
test("goBack calls history.back", function() {
  ...
  GameOfForums.KeyboardShortcuts.goBack();
  ok(called, "history.back is called");
});

test("nextSection calls _changeSection with 1", function() {
  var spy = sandbox.spy(GameOfForums.KeyboardShortcuts, '_changeSection');

  GameOfForums.KeyboardShortcuts.nextSection();
...
test("prevSection calls _changeSection with -1", function() {
  ...
  GameOfForums.KeyboardShortcuts.prevSection();
  ok(spy.calledWith(-1), "_changeSection is called with -1");
});
