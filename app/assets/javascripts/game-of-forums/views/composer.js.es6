import userSearch from 'game-of-forums/lib/user-search';
import afterTransition from 'game-of-forums/lib/after-transition';
import loadScript from 'game-of-forums/lib/load-script';
import avatarTemplate from 'game-of-forums/lib/avatar-template';
import positioningWorkaround from 'game-of-forums/lib/safari-hacks';
import { linkSeenMentions, fetchUnseenMentions } from 'game-of-forums/lib/link-mentions';

const ComposerView = GameOfForums.View.extend(Ember.Evented, {
  ...
  composeState: function() {
    return this.get('model.composeState') || GameOfForums.Composer.CLOSED;
  ...
  refreshPreview: GameOfForums.debounce(function() {
    if (this.editor) {
  ...
  ensureMaximumDimensionForImagesInPreview() {
    ...
    const style = GameOfForums.Mobile.mobileView ?
                'max-width: 100%; height: auto;' :
                'max-width:' + GameOfForums.SiteSettings.max_image_width + 'px;' +
                'max-height:' + GameOfForums.SiteSettings.max_image_height + 'px;';
  ...
  afterRender() {
    ...
    $('a.onebox', $wmdPreview).each(function(i, e) {
      GameOfForums.Onebox.load(e, refresh);
    });
    ...
    $('#wmd-input').autocomplete({
      ...
      dataSource(term){
        return new Ember.RSVP.Promise(function(resolve) {
          ...
          if (GameOfForums.Emoji.translations[full]) {
            return resolve([GameOfForums.Emoji.translations[full]]);
          }

          const options = GameOfForums.Emoji.search(term, {maxResults: 5});

          return resolve(options);
        }).then(function(list) {
          return list.map(function(i) {
            return {code: i, src: GameOfForums.Emoji.urlFor(i)};
          });
        });
      }
    });
  ...
  initEditor() {
    ...
    this.editor = editor = GameOfForums.Markdown.createEditor({
      lookupAvatarByPostNumber(postNumber) {
        ...
        if (posts) {
          ...
          if (quotedPost) {
            ...
            return GameOfForums.Utilities.tinyAvatar(avatarTemplate(username, uploadId));
          }
        }
      }
    });
    ...
    if (!GameOfForums.Utilities.allowsAttachments()) {
    ...
    const saveDraft = GameOfForums.debounce((function() {
    ...
    const $uploadTarget = $("#reply-control"),
          csrf = GameOfForums.Session.currentProp("csrfToken"),
    ...
    this.messageBus.subscribe("/uploads/composer", upload => {
      if (!cancelledByTheUser) {
        if (upload && upload.url) {
          const markdown = GameOfForums.Utilities.getUploadMarkdown(upload);
          this.addMarkdown(markdown + " ");
        } else {
          GameOfForums.Utilities.displayErrorForUpload(upload);
        }
      }
      // reset upload state
      reset();
    ...
    $uploadTarget.fileupload({
      url: GameOfForums.getURL("/uploads.json?client_id=" + this.messageBus.clientId + "&authenticity_token=" + encodeURIComponent(csrf)),
      dataType: "json",
      pasteZone: $uploadTarget,
    });

    $uploadTarget.on("fileuploadsubmit", (e, data) => {
      const isValid = GameOfForums.Utilities.validateUploadedFiles(data.files);
    ...
    $uploadTarget.on("fileuploadfail", (e, data) => {
      reset();
      if (!cancelledByTheUser) {
        GameOfForums.Utilities.displayErrorForUpload(data);
      }
    });
    ...
    if (GameOfForums.Mobile.mobileView) {
  ...
  addMarkdown(text) {
    const ctrl = $('#wmd-input').get(0),
        caretPosition = GameOfForums.Utilities.caretPosition(ctrl),
    ...
    Em.run.schedule('afterRender', function() {
      GameOfForums.Utilities.setCaretPosition(ctrl, caretPosition + text.length);
    });
  },
  ...
  titleValidation: function() {
    ...
    } else if( titleLength > GameOfForums.SiteSettings.max_topic_title_length ) {
      reason = I18n.t('composer.error.title_too_long', {max: GameOfForums.SiteSettings.max_topic_title_length});
    }

    if( reason ) {
      return GameOfForums.InputValidation.create({ failed: true, reason: reason });
    }
  }.property('model.titleLength', 'model.missingTitleCharacters', 'model.minimumTitleLength'),

  categoryValidation: function() {
    if( !GameOfForums.SiteSettings.allow_uncategorized_topics && !this.get('model.categoryId')) {
      return GameOfForums.InputValidation.create({ failed: true, reason: I18n.t('composer.error.category_missing') });
    }
  ...
  replyValidation: function() {
    ...
    if (replyLength < 1) {
      ...
      let tl = GameOfForums.User.currentProp("trust_level");
      if (tl === 0 || tl === 1) {
    ...
    if (reason) {
      return GameOfForums.InputValidation.create({ failed: true, reason });
    }
