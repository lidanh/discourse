...
export default Ember.ObjectController.extend({
  ...
  /**
    ...
    @observes searchTerm
  **/
  _searchTermChanged: GameOfForums.debounce(function() {
    var self = this;
    GameOfForums.Invite.findInvitedBy(self.get('user'), this.get('searchTerm')).then(function (invites) {
  ...
  **/
  canInviteToForum: function() {
    return GameOfForums.User.currentProp('can_invite_to_forum');
  }.property(),
  ...
  **/
  ...
  actions: {
    ...
    /**
      ...
      @method rescive
      @param {GameOfForums.Invite} invite the invite to rescind.
    **/
    ...
    /**
      ...
      @method reinvite
      @param {GameOfForums.Invite} invite the invite to resend.
    **/
    ...
    loadMore: function() {
      ...
      if (self.get('canLoadMore') && !self.get('invitesLoading')) {
        ...
        GameOfForums.Invite.findInvitedBy(self.get('user'), self.get('searchTerm'), model.invites.length).then(function(invite_model) {
          self.set('invitesLoading', false);
          model.invites.pushObjects(invite_model.invites);
          if(invite_model.invites.length === 0 || invite_model.invites.length < GameOfForums.SiteSettings.invites_per_page) {
