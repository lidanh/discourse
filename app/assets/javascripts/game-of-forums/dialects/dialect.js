...
/**
  ...
  GameOfForums uses the Markdown.js as its main parser. `GameOfForums.Dialect` is the framework
...
var parser = window.BetterMarkdown,
    ...
    dialect = MD.dialects.GameOfForums = DialectHelpers.subclassDialect( MD.dialects.Gruber ),
    initialized = false,
    emitters = [],
    hoisted,
    preProcessors = [],
...
function parseTree(tree, path, insideCounts) {
  ...
  if (tree instanceof Array) {
    ...
    GameOfForums.Dialect.trigger('parseNode', event);
...
GameOfForums.Dialect = {
  ...
  **/
  cook: function(text, opts) {
    ...
    var tree = parser.toHTMLTree(text, 'GameOfForums'),
        result = parser.renderJsonML(parseTree(tree));

    if (opts.sanitize) {
      result = GameOfForums.Markdown.sanitize(result);
  ...
  **/
  ...
  /**
    ...
    ```javascript
      GameOfForums.Dialect.inlineReplace(':)', function (text) {
  ...
  **/
  ...
  /**
    ...
    ```javascript
      GameOfForums.Dialect.inlineRegexp({
  ...
  **/
  ...
  /**
    ...
    ```javascript
      ...
      GameOfForums.Dialect.inlineBetween({
        between: '**',
        wordBoundary: true.
  ...
  **/
  replaceBlock: function(args) {
    this.registerBlock(args.start.toString(), function(block, next) {
      ...
      var linebreaks = dialect.options.traditional_markdown_linebreaks ||
          GameOfForums.SiteSettings.traditional_markdown_linebreaks;
  ...
  /**
    ...
    ```javascript
      ...
      GameOfForums.Dialect.postProcessText(function (text) {
  ...
  **/
  ...
  /**
    ...
    ```javascript
      GameOfForums.Dialect.postProcessTag('code', function (contents) {
...
RSVP.EventTarget.mixin(GameOfForums.Dialect);
