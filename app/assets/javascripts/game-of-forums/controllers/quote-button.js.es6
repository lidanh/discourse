...
export default GameOfForumsController.extend({
  ...
  selectText(postId) {
    ...
    const selectedText = GameOfForums.Utilities.selectedText();
    ...
    range.collapse(!GameOfForums.Mobile.isMobileDevice);
    ...
    Em.run.schedule('afterRender', function() {
      ...
      if (GameOfForums.Mobile.isMobileDevice) {
        topOff = topOff + 20;
  ...
  quoteText() {
    ...
    const quotedText = GameOfForums.Quote.build(post, buffer);
