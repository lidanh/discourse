/**
  Supports GameOfForums's custom @mention syntax for calling out a user in a post.
...
GameOfForums.Dialect.inlineRegexp({
  ...
  emitter: function(matches) {
    ...
    if (mentionLookup && mentionLookup(username.substr(1))) {
      return ['a', {'class': 'mention', href: GameOfForums.getURL("/users/") + username.substr(1).toLowerCase()}, username];
    } else {
