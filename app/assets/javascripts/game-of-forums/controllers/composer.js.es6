...
export default Ember.ObjectController.extend(Presence, {
  ...
  replyAsNewTopicDraft: Em.computed.equal('model.draftKey', GameOfForums.Composer.REPLY_AS_NEW_TOPIC_KEY),
  checkedMessages: false,

  showEditReason: false,
  editReason: null,
  maxTitleLength: GameOfForums.computed.setting('max_topic_title_length'),
  ...
  showWarning: function() {
    if (!GameOfForums.User.currentProp('staff')) { return false; }
  ...
  actions: {
    ...
    importQuote() {
      ...
      if (postId) {
        ...
        return this.store.find('post', postId).then(function(post) {
          const quote = GameOfForums.Quote.build(post, post.get("raw"), {raw: true, full: true});
    ...
    openIfDraft() {
      if (this.get('model.viewDraft')) {
        this.set('model.composeState', GameOfForums.Composer.OPEN);
      }
    },
  ...
  appendText(text, opts) {
    ...
    if (c) {
      ...
      if (wmd[0]) {
        Em.run.next(() => GameOfForums.Utilities.setCaretPosition(wmd[0], caret));
      }
    }
  },
  ...
  toggle() {
    ...
    switch (this.get('model.composeState')) {
      case GameOfForums.Composer.OPEN:
        ...
        break;
      case GameOfForums.Composer.DRAFT:
        this.set('model.composeState', GameOfForums.Composer.OPEN);
        break;
      case GameOfForums.Composer.SAVING:
        this.close();
    }
    return false;
  },
  ...
  save(force) {
    ...
    var staged = false;
    const disableJumpReply = GameOfForums.User.currentProp('disable_jump_reply');
    ...
    const promise = composer.save({
      ...
      const currentUser = GameOfForums.User.current();
      ...
      if (!composer.get('replyingToTopic') || !disableJumpReply) {
        ...
        if (post && !staged) {
          GameOfForums.URL.routeTo(post.get('url'));
        }
      }
    ...
    Em.run.schedule('afterRender', function() {
      if (staged && !disableJumpReply) {
        ...
        GameOfForums.URL.jumpToPost(postNumber, { skipIfOnScreen: true });
  ...
  findSimilarTopics() {
    ...
    if (body.length < GameOfForums.SiteSettings.min_body_similar_length) { return; }
    if (title.length < GameOfForums.SiteSettings.min_title_similar_length) { return; }
    ...
    if (!message) {
      message = GameOfForums.ComposerMessage.create({
  ...
  /**
    ...
    @param {Object} opts Options for creating a post
      ...
      @param {GameOfForums.Post} [opts.post] The post we're replying to
      @param {GameOfForums.Topic} [opts.topic] The topic we're replying to
  ...
  **/
  open(opts) {
    ...
    if (composerModel &&
        ...
        composerModel.composeState === GameOfForums.Composer.DRAFT) {
      this.close();
      composerModel = null;
    }
    ...
    return new Ember.RSVP.Promise(function(resolve, reject) {
      if (composerModel && composerModel.get('replyDirty')) {
        ...
        if (composerModel.get('composeState') === GameOfForums.Composer.OPEN &&
            composerModel.get('draftKey') === opts.draftKey) {
          return resolve();
        }

        // If it's the same draft, just open it up again.
        if (composerModel.get('composeState') === GameOfForums.Composer.DRAFT &&
            composerModel.get('draftKey') === opts.draftKey) {
          composerModel.set('composeState', GameOfForums.Composer.OPEN);
          return resolve();
        }
      ...
      if (opts.draftSequence === void 0) {
        return GameOfForums.Draft.get(opts.draftKey).then(function(data) {
  ...
  _setModel(composerModel, opts) {
    if (opts.draft) {
      composerModel = GameOfForums.Composer.loadDraft(opts);
      if (composerModel) {
    ...
    if (opts.topicCategoryId) {
      ...
      if (!splitCategory[1]) {
        ...
        const categories = GameOfForums.Category.list();
  ...
  viewNewReply() {
    GameOfForums.URL.routeTo(this.get('model.createdPost.url'));
    this.close();
    return false;
  },
  ...
  destroyDraft() {
    ...
    if (key) {
      GameOfForums.Draft.clear(key, this.get('model.draftSequence'));
    }
  },
  ...
  collapse() {
    this.saveDraft();
    this.set('model.composeState', GameOfForums.Composer.DRAFT);
  },
  ...
  showOptions() {
    var _ref;
    return (_ref = this.get('controllers.modal')) ? _ref.show(GameOfForums.ArchetypeOptionsModalView.create({
  ...
  canEdit: function() {
    return this.get("model.action") === "edit" && GameOfForums.User.current().get("can_edit");
