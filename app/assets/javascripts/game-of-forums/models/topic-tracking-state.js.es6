...
const TopicTrackingState = GameOfForums.Model.extend({
  messageCount: 0,
  ...
  establishChannels() {
    ...
    const process = function(data){
      ...
      if (data.message_type === "new_topic" || data.message_type === "latest") {
        const ignored_categories = GameOfForums.User.currentProp("muted_category_ids");
...
TopicTrackingState.reopenClass({
  createFromStates(data) {
    ...
    const container = GameOfForums.__container__,
          messageBus = container.lookup('message-bus:main'),
          currentUser = container.lookup('current-user:main'),
          instance = GameOfForums.TopicTrackingState.create({ messageBus, currentUser });
