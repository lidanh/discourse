...
const Topic = RestModel.extend({
  ...
  fancyTitle: function() {
    ...
    if (GameOfForums.SiteSettings.enable_emoji && title.indexOf(":") >= 0) {
      title = title.replace(/:[^\s:]+:?/g, function(m) {
        const emoji = GameOfForums.Emoji.translations[m] ? GameOfForums.Emoji.translations[m] : m.slice(1, m.length - 1),
              url = GameOfForums.Emoji.urlFor(emoji);
  ...
  bumpedAtTitle: function() {
    return I18n.t('first_post') + ": " + GameOfForums.Formatter.longDate(this.get('createdAt')) + "\n" +
           I18n.t('last_post') + ": " + GameOfForums.Formatter.longDate(this.get('bumpedAt'));
  }.property('bumpedAt'),
  ...
  _categoryIdChanged: function() {
    this.set('category', GameOfForums.Category.findById(this.get('category_id')));
  ...
  _categoryNameChanged: function() {
    ...
    let category;
    if (categoryName) {
      category = GameOfForums.Category.list().findProperty('name', categoryName);
    }
  ...
  url: function() {
    ...
    return GameOfForums.getURL("/t/") + slug + "/" + (this.get('id'));
  }.property('id', 'slug'),
  ...
  lastPosterUrl: function() {
    return GameOfForums.getURL("/users/") + this.get("last_poster.username");
  ...
  displayNewPosts: function() {
    const highestSeen = GameOfForums.Session.currentProp('highestSeenByTopic')[this.get('id')];
    if (highestSeen) {
  ...
  viewsHeat: function() {
    ...
    if( v >= GameOfForums.SiteSettings.topic_views_heat_high )   return 'heatmap-high';
    if( v >= GameOfForums.SiteSettings.topic_views_heat_medium ) return 'heatmap-med';
    if( v >= GameOfForums.SiteSettings.topic_views_heat_low )    return 'heatmap-low';
    return null;
  }.property('views'),

  archetypeObject: function() {
    return GameOfForums.Site.currentProp('archetypes').findProperty('id', this.get('archetype'));
  }.property('archetype'),
  ...
  saveStatus(property, value) {
    ...
    return GameOfForums.ajax(this.get('url') + "/status", {
      type: 'PUT',
  ...
  makeBanner() {
    const self = this;
    return GameOfForums.ajax('/t/' + this.get('id') + '/make-banner', { type: 'PUT' })
           .then(function () { self.set('archetype', 'banner'); });
  },

  removeBanner() {
    const self = this;
    return GameOfForums.ajax('/t/' + this.get('id') + '/remove-banner', { type: 'PUT' })
  ...
  toggleBookmark() {
    ...
    const toggleBookmarkOnServer = function() {
      return GameOfForums.ajax('/t/' + self.get('id') + path, {
        type: 'PUT',
      }).then(function() {
  ...
  createInvite(emailOrUsername, groupNames) {
    return GameOfForums.ajax("/t/" + this.get('id') + "/invite", {
      type: 'POST',
  ...
  destroy(deleted_by) {
    ...
    return GameOfForums.ajax("/t/" + this.get('id'), {
  ...
  recover() {
    ...
    return GameOfForums.ajax("/t/" + this.get('id') + "/recover", { type: 'PUT' });
  },
  ...
  clearPin() {
    ...
    GameOfForums.ajax("/t/" + this.get('id') + "/clear-pin", {
      type: 'PUT'
    }).then(null, function() {
  ...
  rePin() {
    ...
    GameOfForums.ajax("/t/" + this.get('id') + "/re-pin", {
      type: 'PUT'
    }).then(null, function() {
      // On error, put the pin back
      topic.set('pinned', true);
  ...
  readLastPost: GameOfForums.computed.propertyEqual('last_read_post_number', 'highest_post_number'),
...
Topic.reopenClass({
  ...
  update(topic, props) {
    ...
    return GameOfForums.ajax(topic.get('url'), { type: 'PUT', data: props }).then(function(result) {
  ...
  find(topicId, opts) {
    let url = GameOfForums.getURL("/t/") + topicId;
    if (opts.nearPost) {
      url += "/" + opts.nearPost;
    }
    ...
    return GameOfForums.ajax(url + ".json", {data: data});
  },

  mergeTopic(topicId, destinationTopicId) {
    const promise = GameOfForums.ajax("/t/" + topicId + "/merge-topic", {
      type: 'POST',
  ...
  movePosts(topicId, opts) {
    const promise = GameOfForums.ajax("/t/" + topicId + "/move-posts", {
      type: 'POST',
      data: opts
    }).then(function (result) {
      if (result.success) return result;
  ...
  changeOwners(topicId, opts) {
    const promise = GameOfForums.ajax("/t/" + topicId + "/change-owner", {
      type: 'POST',
      data: opts
    }).then(function (result) {
      if (result.success) return result;
  ...
  bulkOperation(topics, operation) {
    return GameOfForums.ajax("/topics/bulk", {
      type: 'PUT',
      data: {
  ...
  resetNew() {
    return GameOfForums.ajax("/topics/reset-new", {type: 'PUT'});
  },

  idForSlug(slug) {
    return GameOfForums.ajax("/t/id_for/" + slug);
  }
});
