...
export default ObjectController.extend(SelectedPostsCount, BufferedContent, {
  ...
  actions: {
    ...
    replyToPost(post) {
      const composerController = this.get('controllers.composer'),
          ...
          quotedText = GameOfForums.Quote.build(quoteController.get('post'), quoteController.get('buffer')),
          topic = post ? post.get('topic') : this.get('model');

      quoteController.set('buffer', '');

      if (composerController.get('content.topic.id') === topic.get('id') &&
          composerController.get('content.action') === GameOfForums.Composer.REPLY) {
        composerController.set('content.post', post);
        composerController.set('content.composeState', GameOfForums.Composer.OPEN);
        composerController.appendText(quotedText);
      } else {

        const opts = {
          action: GameOfForums.Composer.REPLY,
      ...
      if (user.get('staff') && replyCount > 0) {
        bootbox.dialog(I18n.t("post.controls.delete_replies.confirm", {count: replyCount}), [
          ...
          {label: I18n.t("post.controls.delete_replies.yes_value"),
           'class': 'btn-primary',
            callback() {
              GameOfForums.Post.deleteMany([post], [post]);
    ...
    editPost(post) {
      ...
      this.get('controllers.composer').open({
        post: post,
        action: GameOfForums.Composer.EDIT,
    ...
    deleteSelected() {
      ...
      bootbox.confirm(I18n.t("post.delete.confirm", { count: this.get('selectedPostsCount')}), function(result) {
        if (result) {
          ...
          GameOfForums.Post.deleteMany(selectedPosts, selectedReplies);
    ...
    replyAsNewTopic(post) {
      ...
      composerController.open({
        action: GameOfForums.Composer.CREATE_TOPIC,
        draftKey: GameOfForums.Composer.REPLY_AS_NEW_TOPIC_KEY,
        categoryId: this.get('category.id')
      }).then(function() {
        return Em.isEmpty(quotedText) ? GameOfForums.Post.loadQuote(post.get('id')) : quotedText;
      }).then(function(q) {
  ...
  canChangeOwner: function() {
    if (!GameOfForums.User.current() || !GameOfForums.User.current().admin) return false;
  ...
  showStarButton: function() {
    return GameOfForums.User.current() && !this.get('model.isPrivateMessage');
  ...
  deleteTopic() {
    this.unsubscribe();
    this.get('content').destroy(GameOfForums.User.current());
  },
  ...
  /**
    ...
    @params {GameOfForums.Post} post that is at the bottom
  **/
