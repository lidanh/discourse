...
function findTopicList(store, filter, filterParams, extras) {
  ...
  return new Ember.RSVP.Promise(function(resolve) {
    const session = GameOfForums.Session.current();
    ...
    const findParams = {};
    GameOfForums.SiteSettings.top_menu.split('|').forEach(function (i) {
    ...
    GameOfForums.Session.currentProp('topicList', list);
    return list;
  });
}
...
export default function(filter, extras) {
  ...
  return GameOfForums.Route.extend({
    ...
    titleToken() {
      if (filter === GameOfForums.Utilities.defaultHomepage()) { return; }
