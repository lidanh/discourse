...
export default RestModel.extend({
  ...
  removeAction: function() {
    ...
    if (this.get('usersExpanded')) {
      this.get('users').removeObject(GameOfForums.User.current());
    }
  },
  ...
  act: function(post, opts) {
    ...
    if (this.get('usersExpanded')) {
      this.get('users').addObject(GameOfForums.User.current());
    }

    // Create our post action
    var self = this;

    return GameOfForums.ajax("/post_actions", {
      type: 'POST',
      data: {
  ...
  undo: function(post) {
    ...
    return GameOfForums.ajax("/post_actions/" + post.get('id'), {
      type: 'DELETE',
      data: {
        post_action_type_id: this.get('id')
      }
  ...
  deferFlags: function(post) {
    var self = this;
    return GameOfForums.ajax("/post_actions/defer_flags", {
      type: "POST",
      data: {
        post_action_type_id: this.get("id"),
        id: post.get('id')
  ...
  loadUsers: function(post) {
    var self = this;
    GameOfForums.ajax("/post_actions/users", {
      data: {
        id: post.get('id'),
        post_action_type_id: this.get('id')
      }
      ...
      _.each(result,function(user) {
        if (user.id === GameOfForums.User.currentProp('id')) {
          users.pushObject(GameOfForums.User.current());
        } else {
          users.pushObject(GameOfForums.User.create(user));
        }
      });
    });
  }
