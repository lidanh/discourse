...
moduleFor("controller:site-map", "controller:site-map", {
  ...
  setup: function() {
    oldMobileView = GameOfForums.Mobile.mobileView;
  },

  teardown: function() {
    GameOfForums.Mobile.mobileView = oldMobileView;
  }
});
...
test("faqUrl returns faq url configured in site settings if it is set", function() {
  GameOfForums.SiteSettings.faq_url = "faq-url";
...
test("faqUrl returns default '/faq' url when there is no corresponding site setting set", function() {
  GameOfForums.SiteSettings.faq_url = null;
  var controller = this.subject();
  equal(controller.get("faqUrl"), "/faq");
});

test("showMoblieToggle returns true when mobile theme is enabled in site settings", function() {
  GameOfForums.SiteSettings.enable_mobile_theme = true;
  GameOfForums.Mobile.isMobileDevice = true;
  var controller = this.subject();
...
test("showMoblieToggle returns false when mobile theme is disabled in site settings", function() {
  GameOfForums.SiteSettings.enable_mobile_theme = false;
  GameOfForums.Mobile.isMobileDevice = true;
  var controller = this.subject();
...
test("categories", function() {
  ...
  sandbox.stub(GameOfForums.Category, "list").returns(categoryListStub);

  var controller = this.subject({ siteSettings: GameOfForums.SiteSettings });
  deepEqual(controller.get("categories"), categoryListStub, "returns the list of categories");
});

test("toggleMobleView", function() {
  sandbox.stub(GameOfForums.Mobile, "toggleMobileView");

  var controller = this.subject();
  controller.send("toggleMobileView");
  ok(GameOfForums.Mobile.toggleMobileView.calledOnce, "switches between desktop and mobile views");
});
