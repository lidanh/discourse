...
function uploadTranslate(key, options) {
  const opts = options || {};
  if (GameOfForums.Utilities.allowsAttachments()) { key += "_with_attachments"; }
...
export default ModalBodyView.extend({
  ...
  uploadIcon: function() { return GameOfForums.Utilities.allowsAttachments() ? "fa-upload" : "fa-picture-o"; }.property(),
  ...
  tip: function() {
    const source = this.get("controller.local") ? "local" : "remote",
          opts = { authorized_extensions: GameOfForums.Utilities.authorizedExtensions() };
