...
const TopicDetails = RestModel.extend({
  ...
  updateFromJson(details) {
    ...
    if (details.allowed_users) {
      details.allowed_users = details.allowed_users.map(function (u) {
        return GameOfForums.User.create(u);
      });
    }
  ...
  notificationReasonText: function() {
    ...
    return I18n.t(localeString, { username: GameOfForums.User.currentProp('username_lower') });
  ...
  updateNotifications(v) {
    ...
    return GameOfForums.ajax("/t/" + (this.get('topic.id')) + "/notifications", {
      type: 'POST',
  ...
  removeAllowedUser(user) {
    ...
    GameOfForums.ajax("/t/" + this.get('topic.id') + "/remove-allowed-user", {
      type: 'PUT',
