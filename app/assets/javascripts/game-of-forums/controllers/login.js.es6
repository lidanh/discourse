...
export default GameOfForumsController.extend(ModalFunctionality, {
  ...
  canLoginLocal: GameOfForums.computed.setting('enable_local_logins'),
  ...
  **/
  hasAtLeastOneLoginButton: function() {
    return Em.get("GameOfForums.LoginMethod.all").length > 0;
  }.property("GameOfForums.LoginMethod.all.@each"),
  ...
  actions: {
    ...
    externalLogin: function(loginMethod){
      ...
      if(customLogin){
        ...
        var w = window.open(GameOfForums.getURL("/auth/" + name), "_blank",
  ...
  authMessage: (function() {
    ...
    var method = GameOfForums.get('LoginMethod.all').findProperty("name", this.get("authenticate"));
    if(method){
  ...
  authenticationComplete: function(options) {
    ...
    if (options.authenticated) {
      if (window.location.pathname === GameOfForums.getURL('/login')) {
