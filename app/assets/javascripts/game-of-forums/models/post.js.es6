...
const Post = RestModel.extend({
  ...
  shareUrl: function() {
    const user = GameOfForums.User.current();
  ...
  hasTimeGap: function() {
    return (this.get('daysSincePrevious') || 0) > GameOfForums.SiteSettings.show_time_gap_days;
  }.property('daysSincePrevious'),

  showName: function() {
    const name = this.get('name');
    return name && (name !== this.get('username'))  && GameOfForums.SiteSettings.display_name_on_posts;
  ...
  url: function() {
    return GameOfForums.Utilities.postUrl(this.get('topic.slug') || this.get('topic_slug'), this.get('topic_id'), this.get('post_number'));
  }.property('post_number', 'topic_id', 'topic.slug'),

  usernameUrl: GameOfForums.computed.url('username', '/users/%@'),

  showUserReplyTab: function() {
    return this.get('reply_to_user') && (
      !GameOfForums.SiteSettings.suppress_reply_directly_above ||
      this.get('reply_to_post_number') < (this.get('post_number') - 1)
    );
  }.property('reply_to_user', 'reply_to_post_number', 'post_number'),

  topicOwner: GameOfForums.computed.propertyEqual('topic.details.created_by.id', 'user_id'),
  hasHistory: Em.computed.gt('version', 1),
  postElementId: GameOfForums.computed.fmt('post_number', 'post_%@'),

  canViewRawEmail: function() {
    return this.get("user_id") === GameOfForums.User.currentProp("id") || GameOfForums.User.currentProp('staff');
  }.property("user_id"),
  ...
  _updatePost(field, data) {
    const self = this;
    GameOfForums.ajax("/posts/" + this.get("id") + "/" + field, {
      type: "PUT",
      data: data
    }).then(function () {
  ...
  flagsAvailable: function() {
    const post = this;
    return GameOfForums.Site.currentProp('flagTypes').filter(function(item) {
  ...
  afterUpdate(res) {
    if (res.category) {
      GameOfForums.Site.current().updateCategory(res.category);
    }
  },
  ...
  createProperties() {
    const data = this.getProperties(GameOfForums.Composer.serializedFieldsForCreate());
  ...
  expand() {
    const self = this;
    return GameOfForums.ajax("/posts/" + this.get('id') + "/expand-embed").then(function(post) {
  ...
  recover() {
    ...
    return GameOfForums.ajax("/posts/" + (this.get('id')) + "/recover", { type: 'PUT', cache: false }).then(function(data){
      post.setProperties({
        cooked: data.cooked,
        raw: data.raw,
        user_deleted: false,
  ...
  **/
  setDeletedState(deletedBy) {
    ...
    if (deletedBy.get('staff') || GameOfForums.SiteSettings.delete_removed_posts_after === 0) {
      this.setProperties({
        deleted_at: new Date(),
      ...
      this.setProperties({
        cooked: GameOfForums.Markdown.cook(I18n.t("post.deleted_by_author", {count: GameOfForums.SiteSettings.delete_removed_posts_after})),
        can_delete: false,
  ...
  **/
  ...
  destroy(deletedBy) {
    ...
    return GameOfForums.ajax("/posts/" + this.get('id'), {
  ...
  **/
  ...
  loadReplies() {
    ...
    const self = this;
    return GameOfForums.ajax("/posts/" + (this.get('id')) + "/replies")
      .then(function(loaded) {
        const replies = self.get('replies');
        _.each(loaded,function(reply) {
          const post = GameOfForums.Post.create(reply);
  ...
  showRepliesBelow: function() {
    ...
    if (!GameOfForums.SiteSettings.suppress_reply_directly_below) return true;
  ...
  expandHidden() {
    const self = this;
    return GameOfForums.ajax("/posts/" + this.get('id') + "/cooked.json").then(function (result) {
      self.setProperties({
  ...
  rebake() {
    return GameOfForums.ajax("/posts/" + this.get("id") + "/rebake", { type: "PUT" });
  },

  unhide() {
    return GameOfForums.ajax("/posts/" + this.get("id") + "/unhide", { type: "PUT" });
  },
  ...
  toggleBookmark() {
    ...
    return GameOfForums.Post.updateBookmark(this.get('id'), this.get('bookmarked'))
...
Post.reopenClass({
  ...
  munge(json) {
    if (json.actions_summary) {
      ...
      json.actions_summary = json.actions_summary.map(function(a) {
        a.actionType = GameOfForums.Site.current().postActionTypeById(a.id);
        const actionSummary = GameOfForums.ActionSummary.create(a);
    ...
    if (json && json.reply_to_user) {
      json.reply_to_user = GameOfForums.User.create(json.reply_to_user);
    }
    return json;
  },

  updateBookmark(postId, bookmarked) {
    return GameOfForums.ajax("/posts/" + postId + "/bookmark", {
      type: 'PUT',
  ...
  deleteMany(selectedPosts, selectedReplies) {
    return GameOfForums.ajax("/posts/destroy_many", {
      type: 'DELETE',
      data: {
  ...
  loadRevision(postId, version) {
    return GameOfForums.ajax("/posts/" + postId + "/revisions/" + version + ".json").then(function (result) {
      return Ember.Object.create(result);
    });
  },

  hideRevision(postId, version) {
    return GameOfForums.ajax("/posts/" + postId + "/revisions/" + version + "/hide", { type: 'PUT' });
  },

  showRevision(postId, version) {
    return GameOfForums.ajax("/posts/" + postId + "/revisions/" + version + "/show", { type: 'PUT' });
  },

  loadQuote(postId) {
    return GameOfForums.ajax("/posts/" + postId + ".json").then(function (result) {
      const post = GameOfForums.Post.create(result);
      return GameOfForums.Quote.build(post, post.get('raw'), {raw: true, full: true});
    });
  },

  loadRawEmail(postId) {
    return GameOfForums.ajax("/posts/" + postId + "/raw-email").then(function (result) {
