export default {
  trackClick(e) {
    ...
    if (GameOfForums.Utilities.selectedText() !== "") { return false; }
    ...
    var ownLink = userId && (userId === GameOfForums.User.currentProp('id')),
        trackingUrl = GameOfForums.getURL("/clicks/track?url=" + encodeURIComponent(href));
    ...
    if (e.which === 3) {
      var destination = GameOfForums.SiteSettings.track_external_right_clicks ? trackingUrl : href;
    ...
    if (e.shiftKey || e.metaKey || e.ctrlKey || e.which === 2) {
      GameOfForums.ajax("/clicks/track", {
        data: {
          url: href,
          post_id: postId,
          topic_id: topicId,
    ...
    if (GameOfForums.SiteSettings.prevent_anons_from_downloading_files && $link.hasClass("attachment") && !GameOfForums.User.current()) {
      bootbox.alert(I18n.t("post.errors.attachment_download_requires_login"));
      return false;
    }

    // If we're on the same site, use the router and track via AJAX
    if (GameOfForums.URL.isInternal(href) && !$link.hasClass('attachment')) {
      GameOfForums.ajax("/clicks/track", {
        data: {
          url: href,
          post_id: postId,
      ...
      GameOfForums.URL.routeTo(href);
      return false;
    }

    // Otherwise, use a custom URL with a redirect
    if (GameOfForums.User.currentProp('external_links_in_new_tab')) {
      var win = window.open(trackingUrl, '_blank');
      win.focus();
    } else {
      GameOfForums.URL.redirectTo(trackingUrl);
    }
