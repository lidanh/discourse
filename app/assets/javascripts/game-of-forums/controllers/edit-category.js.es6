...
export default ObjectController.extend(ModalFunctionality, {
  ...
  parentCategories: function() {
    return GameOfForums.Category.list().filter(function (c) {
  ...
  subCategories: function() {
    ...
    return GameOfForums.Category.list().filterBy('parent_category_id', this.get('model.id'));
  }.property('model.id'),
  ...
  emailInEnabled: GameOfForums.computed.setting('email_in'),
  ...
  categoryBadgePreview: function() {
    ...
    const c = GameOfForums.Category.create({
  ...
  backgroundColors: function() {
    ...
    return GameOfForums.SiteSettings.category_colors.split("|").map(function(i) { return i.toUpperCase(); }).concat(
                categories.map(function(c) { return c.color.toUpperCase(); }) ).uniq();
  }.property('GameOfForums.SiteSettings.category_colors'),
  ...
  showPositionInput: GameOfForums.computed.setting('fixed_category_positions'),

  actions: {
    showCategoryTopic() {
      this.send('closeModal');
      GameOfForums.URL.routeTo(this.get('model.topic_url'));
      return false;
    },
    ...
    addPermission(group, id) {
      this.get('model').addPermission({group_name: group + "",
                                       permission: GameOfForums.PermissionType.create({id})});
    },
    ...
    saveCategory() {
      const self = this,
          ...
          parentCategory = GameOfForums.Category.list().findBy('id', parseInt(model.get('parent_category_id'), 10));
      ...
      this.get('model').save().then(function(result) {
        ...
        GameOfForums.URL.redirectTo("/c/" + GameOfForums.Category.slugFor(model));
      ...
      bootbox.confirm(I18n.t("category.delete_confirm"), I18n.t("no_value"), I18n.t("yes_value"), function(result) {
        if (result) {
          self.get('model').destroy().then(function(){
            // success
            self.send('closeModal');
            GameOfForums.URL.redirectTo("/categories");
          }, function(error){
